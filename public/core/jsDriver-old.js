/**
1. 嵌入
fetch('http://localhost:3001/money-chat/api/1.0.0/base').then(res => res.json()).then(json => {
...
2. 直接访问
http://localhost:3001/money-chat
*/
init();
        ...
        if (msg.msgType == 21) {
            recvMsg = `<div class="mc-media text-center">
                  <div class="mc-media-content" style="width: 100%">
                      <small class="text-muted">欢迎，${msg.msg} 加入聊天室！</small>
                  </div>
              </div>`;;
            // 有人退出
            ...
            recvMsg = `<div class="mc-media text-center">
                  <div class="mc-media-content" style="width: 100%">
                      <small class="text-muted">恭送，${msg.msg}离开聊天室！</small>
                  </div>
              </div>`;
            // 有人群里发信息
        } else {
...
// 发送信息
document.getElementById('moneyChatInput').addEventListener('keydown', function (event) {
    if (event.keyCode === 13) {
        $sendMsg()
        event.preventDefault();
    }
...
function createChatSession(username, name, isRoom) {
    ...
    if (isRoom) {
        session.querySelector('.user-avatar').src = 'https://i.loli.net/2020/10/24/p4eVy3sKDkYGEbn.jpg';
...
// 打开聊天会话
function openSession(thisObj, isRoom) {
...
function $openSession(to, toName, isRoom) {
    ...
    if (isRoom) {
        chatWindow.querySelector('.mc-header .user-avatar').src =
            'https://i.loli.net/2020/10/24/p4eVy3sKDkYGEbn.jpg';
    }
